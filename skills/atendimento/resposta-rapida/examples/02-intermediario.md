Resposta com solicitação de complemento.

## Contexto

Esta e a entrada esperada para a Skill `atendimento/resposta-rapida` no nivel **intermediario**.

Descricao da Skill: Use para gerar resposta curta, cordial e útil a demandas de atendimento.

## Saida esperada

Producao coerente com o objetivo da Skill, respeitando o tom corporativo e o vocabulario bloqueado definidos em `config/empresa.yaml`.

## Verificacoes

- Conteudo cobre o objetivo declarado.
- Tom e linguagem estao consistentes.
- Campos obrigatorios estao presentes.
